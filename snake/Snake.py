...
def draw_apple(stdscr, y, x, color_pair):
    try:
        stdscr.addstr(y, x, "⌺", color_pair | curses.A_BOLD)
    except curses.error:
        pass

def draw_watermark(stdscr, y, x_end):
    """
    Renders the split-colored watermark: Green text with Blue '@67'
    """
    g_term = curses.color_pair(1)
    b_term = curses.color_pair(2)
    
    text_part1 = "Developed by @Codog "
    text_part2 = "@67"
    total_len = len(text_part1) + len(text_part2)
    
    # Calculate starting position based on trailing right alignment
    start_x = x_end - total_len
    
    try:
        stdscr.addstr(y, start_x, text_part1, g_term | curses.A_BOLD)
        stdscr.addstr(y, start_x + len(text_part1), text_part2, b_term | curses.A_BOLD)
    except curses.error:
        pass
...
def pause_menu(stdscr, g_term):
    ...
    # Updated Watermark inside Pause Screen
    m_height, m_width = stdscr.getmaxyx()
    draw_watermark(stdscr, m_height - 1, m_width - 2)
...
def show_game_over(win, score):
    ...
    line1 = "GAME OVER"
    line2 = f"Final Score: {score} BLOCKS"
    line3 = "Press [R] to Restart or [Q] to Main Menu"

    # Display elements beautifully aligned
    win.addstr(center_y - 2, center_x - (len(line1) // 2), line1, curses.color_pair(1) | curses.A_BOLD)
    win.addstr(center_y, center_x - (len(line2) // 2), line2, curses.A_BOLD)
    win.addstr(center_y + 2, center_x - (len(line3) // 2), line3, curses.A_DIM)

    # Added watermarks to the game-over screen interface context block
    draw_watermark(win, height - 2, width - 2)
    win.refresh()
...
def game_loop(stdscr):
    ...
    while True:
        ...
        for x in range(box_left + 1, box_right):
            stdscr.addch(box_top, x, '─')
            stdscr.addch(box_bottom, x, '─')
            
        # Updated Main Dynamic Watermark
        draw_watermark(stdscr, box_bottom + 1, box_right)

        for y in range(box_top + 1, box_bottom):
            stdscr.addch(y, box_left, '│')
            stdscr.addch(y, box_right, '│')
            
        stdscr.addch(box_top, box_left, '┌')
        stdscr.addch(box_top, box_right, '┐')
        stdscr.addch(box_bottom, box_left, '└')
        stdscr.addch(box_bottom, box_right, '┘')
        
        if food:
            draw_apple(stdscr, food[0], food[1], g_term)
        ...
        if key == 27:
            ...
            if pause_menu(stdscr, g_term) == "QUIT":
                return snake_size, elapsed_seconds
            paused_duration += (time.time() - pause_start)
            stdscr.timeout(40)
            continue
            
        if key in [curses.KEY_UP, ord('w'), ord('W')] and current_dir != DIR_DOWN:
            next_dir = DIR_UP
        elif key in [curses.KEY_DOWN, ord('s'), ord('S')] and current_dir != DIR_UP:
            next_dir = DIR_DOWN
        elif key in [curses.KEY_LEFT, ord('a'), ord('A')] and current_dir != DIR_RIGHT:
            next_dir = DIR_LEFT
        elif key in [curses.KEY_RIGHT, ord('d'), ord('D')] and current_dir != DIR_LEFT:
            next_dir = DIR_RIGHT
            
        tick_counter += 1
        if next_dir in [DIR_UP, DIR_DOWN] and tick_counter % 2 == 0:
            continue
...
def master_flow(stdscr):
    # Initialize Terminal Colors Once Global Setup
    curses.start_color()
    curses.init_pair(1, curses.COLOR_GREEN, curses.COLOR_BLACK)  # Pair 1: Green elements
    curses.init_pair(2, curses.COLOR_BLUE, curses.COLOR_BLACK)   # Pair 2: Custom Blue for "@67"
    
    score, survival_time = None, None
    while True:
        if not main_menu(stdscr, score, survival_time):
            break
            
        # Play game round
        score, survival_time = game_loop(stdscr)
        
        # Open game over sequence before deciding loop path redirection
        restart_choice = show_game_over(stdscr, score)
        if not restart_choice:
            # Drop cleanly out back into main dashboard menu instead of closing app
            continue
